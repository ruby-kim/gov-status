...
export default function AnalyticsTable({ agencyStats }: AnalyticsTableProps) {
  ...
  const [sortField, setSortField] = useState<'agency' | 'current' | 'day1' | 'week1' | 'month1'>(
    'current'
  );
  const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('desc');
  const [filterStatus, setFilterStatus] = useState<'all' | 'improved' | 'declined' | 'stable'>(
    'all'
  );
    ...
    .filter((agency) => {
      ...
      if (filterStatus !== 'all') {
        ...
        if (
          filterStatus === 'stable' &&
          yesterdayRate &&
          Math.abs(currentRate - yesterdayRate) > 0.1
        ) {
          return false;
        }
      }
    ...
    <div className="w-full">
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-4 lg:p-6 text-xs sm:text-sm md:text-base lg:text-lg w-full">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">기관별 정상율 상세 통계</h3>
                ...
                <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-300 rounded-md shadow-lg z-10">
                  {filterOptions.map((option) => (
                    <button
                      ...
                      onClick={() =>
                        handleFilterChange(
                          option.value as 'all' | 'improved' | 'declined' | 'stable'
                        )
                      }
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paginatedAgencyStats.map((agency) => {
                ...
                return (
                  ...
                  >
                    <td
                      className="px-2 sm:px-3 lg:px-4 py-2 sm:py-2 lg:py-3 font-medium text-gray-900"
                    ...
                    <td
                      className={`px-2 sm:px-3 lg:px-4 py-2 sm:py-2 lg:py-3 text-center font-semibold ${getCurrentRateColor()}`}
                    >
                    ...
                    <td className="px-2 sm:px-3 lg:px-4 py-2 sm:py-2 lg:py-3 text-center text-gray-700">
                      {agency.day1.normalRate !== null ? (
                        formatPercentage(agency.day1.normalRate)
                      ) : (
                        <span className="text-gray-400">N/A</span>
                      )}
                    </td>
                    <td className="px-2 sm:px-3 lg:px-4 py-2 sm:py-2 lg:py-3 text-center text-gray-700">
                      {agency.week1.normalRate !== null ? (
                        formatPercentage(agency.week1.normalRate)
                      ) : (
                        <span className="text-gray-400">N/A</span>
                      )}
                    </td>
                    <td className="px-2 sm:px-3 lg:px-4 py-2 sm:py-2 lg:py-3 text-center text-gray-700">
                      {agency.month1.normalRate !== null ? (
                        formatPercentage(agency.month1.normalRate)
                      ) : (
                        <span className="text-gray-400">N/A</span>
                      )}
                    </td>
